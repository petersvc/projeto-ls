...
const fetchUser = user => {
    let endpoint = `https://api.github.com/users/${user}` // chamando da api
    const data = fetch(endpoint).then(res => res.json()) // guardando a informação em um json             
    return data                  
}
...
const fetchLang = lang => {
    let endpoint = lang // array com as urls das linguagens, obtido na fetchRepo()
    let data = [] // array que conterá as linguaguens de cada repositório
...
const showData = () => {
    // User data
    fetchUser(searchInput.value).then(res => {
    ...
    // Repositories data
    fetchRepo(searchInput.value).then(res => {
